...
<?php
        ...
        $query = "SELECT S.ID_S as ITEM, R.ELEMENTO_ID as ID, R.ITEM as SUB, R.REL_MONTO, R.dsc_item as DSC2, R.dsc_elem as DSC
					FROM ELEM_SEG S 
					LEFT JOIN RELACION R ON R.ELEMENTO_ID = S.ID
					ORDER BY S.ID_S, R.ELEMENTO_ID, R.ITEM "; //LIMIT 2000";
...
$lennx = isset($posts[$j]['ITEM'])? substr_count($posts[$j]['ITEM'], '.')+1 : 0;
//	$lennx = substr_count($posts[$j]['ITEM'], '.'); // (sizeof($posts)>=$j)? strlen($posts[$j]['ITEM']):strlen($posts[$j]['ITEM']);
	$lencu = substr_count($row['ITEM'], '.')+1; //    strlen($row['ITEM']);
	$text = $row['REL_MONTO'] .'uf'; //$row['ITEM']. ' ex:'. $lenex. ' cu:'. $lencu. ' nx:'. $lennx;
	//$alt = '('. $row['REL_MONTO'] .'/'. $row['COSTO'] .')';  // alt o title
...
//	if( $lencu == $lenex ) {  //HOJA esta y la prox (mismo nivel)
	if( $lencu == $lennx ) {
		echo '<li><a href="#r">', $text, ' ', $row['ITEM'], ' ', $row['DSC'], ' ', $row['SUB'], ' ', $row['DSC2'], '</a></li>';
...
//		for($i=$lencu; $NIVEL[$i-1] >= $lennx; $i--) {      // por aqui es , ya que el nivel no se relaciona con la cantidad de saltos, puede haber niveles saltados hacia arriba
//			echo '<br><b>', $i, ': [', $NIVEL[$i-1] , ']</b><br>';
		  array_pop($NIVEL); //retira el ultimo
		  echo '</ul>';
		  echo '</li>';
//echo '<br>salio CICLO!!!!!!!!!!!   (', $i, ') <br>';
		}
 //               echo '<pre>';
 //               print_r($NIVEL);
 //               echo '</pre>';
...
//		echo '<li><a>RAMA --> ', $text, ' ', $row['ITEM'], ' ', $row['DSC'], '</a>';
		echo '<li><input type="checkbox" name="list" id="', $row['ITEM'].$row['SUB'], '"><label for="', $row['ITEM'].$row['SUB'], '">', $text, ' ', $row['ITEM'], ' ', $row['DSC'], ' ', $row['SUB'], ' ', $row['DSC2'], '</label>';
